...
> 状态：**设计草案**（未实现）  
> 关联：[PRODUCT.md](./PRODUCT.md) §4.2 · [PLAN.md](./PLAN.md) · 本地双人 = 自定义试炼「预设答案」  
...
| P2 | **道具赛 / 特殊规则** | 中段改色/改位等（§2.3 正文）；系列局上叠道具拉长决策 |
...
### 2.3 中段道具 / 道具赛（规格正文 · 仅本文）

> **权威出处**：改色 / 改位等中段道具只在本文设计；Solo / PRODUCT / PLAN 不再展开。  
> 目的：连上后多局里增加决策回合，摊薄建连成本、拉长一盘时长。

#### 中段改色 / 改位（从原 Solo 2 绿阶梯迁入）

| 项 | 草案 |
| ---- | ------ |
| 触发 | 本局某次提交后 `exactCount === 2`（可改为「每局赠 2 AP」等，联机再定） |
| 行动力 | **AP = 2**；用尽或选手动结束 → 回常规猜 |
| `recolor` 改色 | 花 1 AP：改**下一枪草稿**中一格颜色（遵守禁重复等规则） |
| `reshuffle` 改位 | 花 1 AP：交换草稿两格颜色 |
| 作用行 | 拷贝刚提交行为草稿；**不篡改**历史行与反馈 |
| 信息 | **不**额外揭晓哪两格为绿（汇总模式下保持推理） |

```ts
type PropId = 'recolor' | 'reshuffle'
type PropPhaseState = {
...
| 道具 | 触发 | 效果 |
| ------ | ------ | ------ |
| 透视 | 每盘 1 次 | 揭示某一列是否 exact |
| 干扰 / 冻结 | 竞速 | 拖延或短禁对方提交 |

原则：增加决策与节奏变化，避免纯手速。厄运时刻（Fate Night；玩法随主题）见 [PRODUCT.md](./PRODUCT.md)，Duo 是否共用另议。
